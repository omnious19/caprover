...
-   Improved: Allowed custom dockstation domain for webhooks [PR-1330](https://github.com/caprover/caprover/pull/1330)
...
-   Double hyphen in "image name" is now changed to single hyphen, `img-dockstation--myapp` is now changed to `img-dockstation-myapp`. This is to address the problem with 3rd party docker registries. See (https://github.com/caprover/caprover/issues/454). Note that this is only applied to the newly built images. All existing images will stay intact. Also keep in mind that service names and volume names remain the same with double hyphens for now.
...
-   Backup creation was fixed for instances that were migrated from DockStationDuckDuck
...
Awesome news for legacy DockStationDuckDuck users! You can now migrate to CapRover! See this:
...
-   Fall back to `Dockerfile` if `dockstation-definition` file does not exist
...
**Major Update. Renamed from DockStationDuckDuck to CapRover**

Script for upgrading from DockStationDuckDuck to CapRover will be ready in a few weeks, [here](https://caprover.com/docs/cdd-migration.html).

### Breaking Changes:

-   schemaVersion in dockstation-definition file is now upgraded to 2
-   The extra ./src prefix is no longer needed for dockstation-definition dockerfiles. You need to remove ./src from the first argument in your COPY and ADD statements in your dockerfileLines
...
-   Allowing Dockerfile to be referenced in dockstation-definition (https://github.com/caprover/caprover/issues/113)
-   Ability to reference imageName inside dockstation-definition directly when you want o deploy an already built image from DockerHub.
...
-   Ability to simply copy and paste a Dockerfile and dockstation-definition files on web dashboard.
...
-   Fixed the issue with HTTPS repos https://github.com/githubsaturn/dockstationduckduck/issues/283
-   Fixed port mapping disabled on app push
-   Added VSTS Agent as one-click app

## [V0.7.2] - 2018-05-16

Starting V0.7.0, nginx was no longer using the Docker routing mesh, this was done to support showing real IPs behind requests. However, this caused issues on servers that have their ports blocked by firewall, see https://github.com/githubsaturn/dockstationduckduck/issues/237

Although it's not a bug in DockStationDuckDuck, it might cause confusion for new users, hence, a precheck for firewall test is added to the installation phase. If a firewall on important ports is detected, the user will be warned with a proper message and an instruction on how to disable firewall.

## [V0.7.1] - 2018-05-14

Added:

-   Ability to override default values. This includes overriding default DockStation dashboard subdomain and etc.
...
-   Added prechecks on OS and Docker version when installing DockStationDuckDuck to ensure maximum compatibility
...
-   Default client max size for DockStation is changed to 500mb
...
-   Third-party private registries limited via username scope were not correctly getting authenticated (https://github.com/githubsaturn/dockstationduckduck/commit/81fe0408b05dae0d9f4c84c5546c54dfb6845259)
...
-   Make sure that your CLI version is at least `1.0.11`
    -- To check your current version: `dockstationduckduck -V`
    -- To update: `sudo npm install -g dockstationduckduck`
-   Breaking change in your `dockstation-definition` files:
...
```
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-nginx
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-dockstation
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-certbot
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-registry
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-app1   # replace app 1 and app 2 with your app names
docker service update  --log-driver json-file --log-opt max-size=512m  dockstation-app2    # replace app 1 and app 2 with your app names
```

### Features added:

-   Ruby/Rack support added (see https://github.com/githubsaturn/dockstationduckduck/pull/45)
-   Support for showing build logs when deploying via command line added (https://github.com/githubsaturn/dockstationduckduck/commit/b64c3172df5dc8da1970e64d480f9a546ec53851). Simply running `dockstationduckduck deploy` now shows the build logs.
-   Added CouchDB as one-click app

### Improved:

-   Added UDP protocol to Port Forwarding (previously it was only TCP)
-   Container log files are now maxed at 512MB per container by default in order to avoid high unnecessary disk usage. It can be changed by updating service manually, e.g., `docker service update srv-dockstation--app-name --log-driver json-file --log-opt max-size=2048m`
-   DockStation now runs a `docker container prune` to clean up stopped containers
...
-   Added ability to Enorce HTTPS for apps! (https://github.com/githubsaturn/dockstationduckduck/issues/2)
-   Advanced users are now able to customize their nginx configurations (allowing http2, process counts, cache config, custom redirects and etc). https://github.com/githubsaturn/dockstationduckduck/issues/44
-   Added a custom `nginx-shared` directory for using custom files in nginx container (https://github.com/githubsaturn/dockstationduckduck/issues/71). Using this combined with customized nginx config, you can use custom SSL certificates, custom static assets and etc.
...
-   Force HTTPS button on web dashboard now gets disabled if Force HTTPS is activated. (https://github.com/githubsaturn/dockstationduckduck/commit/efc43849e892e6f1032072f4c64317f0946b23af)

CLI (1.0.9):

-   Added a `--default` flag to `dockstationduckduck deploy` to avoid re-entering server address and other information.

## [0.3.1] - 2017-12-27

Fixed:

-   In cluster mode, the initial instance was always getting created on main node due to wrong image tag (https://github.com/githubsaturn/dockstationduckduck/commit/c6e9e4b0c25181ecf9ae7f8eec42379685a6726c)
...
-   Improved deploy process by pre-checking `dockstation-definition` file.
...
-   Added ability to set a custom initial password instead of `dockstation42`
-   Allow customDomain === rootDomain, see https://github.com/githubsaturn/dockstationduckduck/pull/24
...
-   Instance count of `0` is not respected. FIXED. https://github.com/githubsaturn/dockstationduckduck/issues/48#issuecomment-352289264

Breaking Change:

-   If you were manually setting port mapping on your containers before this release, make sure to re-map them through DockStation interface. Since port mapping is added to the interface, DockStation would override the current port mapping that you might have on your containers. If you didn't manually modified your containers `--publish-add` flag, you don't need to worry about this.
...
-   Support for Persistent Data:
    Previously, DockStation only had support for apps with no persistent. The main drawback was that one could not deploy a database using dockstation Now the feature is fully supported.
-   One-click apps:
...
-   Deployment from the web interface now accepts tar archive files of the source directory, not necessarity content. See https://github.com/githubsaturn/dockstationduckduck/issues/22
...
-   Added functionality to disable "Expose As Web App". Now if you are deploying an app that's only being used as an internal service, you have the option not to expose it via a public URL. This is also useful when in the next release DockStation starts supporting databases as they are web-apps at all.
